...
object KeycloakGroupIntegration {
    ...
    private val keycloakRealm = appConfig.property("ktor.keycloak.keycloakRealm")
    ...
    /**
     * Sends a uttaksforesporsel that was built using [buildRequest]
     *
     ...
     * @param contentType A optional [ContentType] that should be used when uttaksforesporsel bodies are used.
     * @param body A optional uttaksforesporsel body to be used when posting or patching
     *
     * @return A [KeycloakIntegrationError] on failure and a Json [String] on success.
     */
    ...
    /**
     * Handles errors that occurs when a uttaksforesporsel is sent and received. Helper function for [sendRequest].
     * @param e A [Throwable] that occurs during a uttaksforesporsel.
     * @return A [KeycloakIntegrationError] that better represents what went wrong during the uttaksforesporsel.
     */
    private fun handleRequestFailure(e: Throwable) = when (e) {
        is ClientRequestException -> when (e.response.status.value) {
            ...
            KeycloakIntegrationError.KeycloakError("Failed to perform Keycloak uttaksforesporsel").left()
        }
    }
    ...
    private suspend fun authenticate() = runCatching {
        ...
        .onFailure { logger.warn("Failed to perform auth uttaksforesporsel") }
        .fold(
            { token = json.parse(TokenResponse.serializer(), it); Unit.right() },
            {
                KeycloakIntegrationError.AuthenticationError("Failed to perform auth uttaksforesporsel")
                    .left()
            }
        )
