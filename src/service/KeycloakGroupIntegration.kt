...
import ombruk.backend.form.KeycloakPartnerForm
import ombruk.backend.model.TokenResponse
...
object KeycloakGroupIntegration {
    ...
    private suspend fun <T> performRequest(function: suspend () -> T): Either<KeycloakIntegrationError, T> {
        ...
        return result ?: KeycloakIntegrationError.KeycloakError("Failed to perform keycloak request").left()
    }
    ...
    private fun getGroups() =
        runBlocking {
            performRequest {
                client.get<String>(groupsUrl) {
                    header("Authorization", "Bearer " + token.accessToken)
                    accept(ContentType.Application.Json)
                }
            }
        }.bimap({ it }, { json.parse(KeycloakPartnerForm.serializer().list, it) })
    ...
    fun updateGroup(oldName: String, newName: String): Either<KeycloakIntegrationError, Unit> =
        getGroupByName(oldName)
            .map {
                runBlocking {
                    performRequest {
                        client.put<String>(groupsUrl + it.id) {
                            header("Authorization", "Bearer " + token.accessToken)
                            accept(ContentType.Application.Json)
    ...
    fun deleteGroup(name: String): Either<KeycloakIntegrationError, Unit> =
        getGroupByName(name)
            .map {
                runBlocking {
                    performRequest {
                        client.delete<String>(groupsUrl + it.id) {
                            header("Authorization", "Bearer " + token.accessToken)
                            accept(ContentType.Application.Json)
                        }
                    }
