...
object PickupRepository : IRepository {
    ...
    fun savePickup(pickupPostForm: PickupPostForm): Either<RepositoryError, Pickup> = runCatching {
        ...
        .fold({ getPickupById(it) }, { RepositoryError.InsertError("SQL error").left() })
    ...
    fun getPickups(pickupQueryForm: PickupGetForm): Either<ServiceError, List<Pickup>> = runCatching {
        transaction {
        ...
        .fold({ it.right() }, { RepositoryError.SelectError("Database query failed").left() })


    fun updatePickup(pickupUpdateForm: PickupUpdateForm): Either<RepositoryError, Pickup> = runCatching {
        transaction {
    ...
    fun deletePickup(id: Int) = runCatching {
        transaction { Pickups.deleteWhere { Pickups.id eq id } }
    }
        .onFailure { logger.error(it.message) }
        .fold({ id.right() }, { RepositoryError.DeleteError("Failed to delete pickup with ID $id").left() })

    override fun exists(id: Int) = transaction { Pickups.select { Pickups.id eq id }.count() >= 1 }
}
