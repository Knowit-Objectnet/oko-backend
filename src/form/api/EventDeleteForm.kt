        ...
        fun create(params: Parameters): Either<ValidationError, EventDeleteForm> {
            ...
            params["event-id"]?.let { param ->
                kotlin.runCatching { param.toInt() }
                    ...
                    .onFailure { return ValidationError.InputError("event-id must be an Int, is: $param").left() }
            }
            params["recurrence-rule-id"]?.let { param ->
                kotlin.runCatching { param.toInt() }
                    .onSuccess { event.recurrenceRuleID = it }
                    .onFailure {
                        return ValidationError.InputError("recurrence-rule-id must be an Int, is: $param").left()
                    }
            }
            if (event.eventID != null && event.recurrenceRuleID != null) return ValidationError.InvalidStateError("event-id and recurrence-rule-id cannot both be set")
                .left()
            if (event.eventID == null && event.recurrenceRuleID == null) return ValidationError.InvalidStateError("Must pass in event-id or recurrence-rule-id")
                .left()
