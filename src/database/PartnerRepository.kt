...
import ombruk.backend.form.PartnerForm
...
object PartnerRepository : IPartnerRepository {
    ...
    override fun insertPartner(partner: PartnerForm) = runCatching {
        ...
        .fold({ Partner(it.value, partner.name).right() }, { RepositoryError.InsertError("SQL error").left() })
    ...
    override fun deletePartner(partnerID: Int) =
        runCatching { Partners.deleteWhere { Partners.id eq partnerID } }
            ...
            .fold(
                { Either.cond(it > 0, { Unit }) { RepositoryError.NoRowsFound("$partnerID not found") } },
    ...
    override fun getPartnerByID(partnerID: Int): Either<RepositoryError.NoRowsFound, Partner> = runCatching {
        Partners.select { Partners.id eq partnerID }.map { Partner(it[Partners.id].value, it[Partners.name]) }
    }
        .onFailure { logger.error(it.message) }
        .fold(
            { Either.cond(it.isNotEmpty(), { it.first() }, { RepositoryError.NoRowsFound("$partnerID not found") }) },
            { RepositoryError.NoRowsFound(it.message).left() })


    override fun getPartners(): Either<RepositoryError.SelectError, List<Partner>> =
        runCatching { Partners.selectAll().map { Partner(it[Partners.id].value, it[Partners.name]) } }
            .onFailure { logger.error(it.message) }
